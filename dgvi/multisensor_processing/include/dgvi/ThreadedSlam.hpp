...
#ifndef INCLUDE_DGVI_THREADEDSLAM3_HPP_
#define INCLUDE_DGVI_THREADEDSLAM3_HPP_
...
class ThreadedSlam : public ViInterface {
 public:
   ...
   */
  ThreadedSlam(dgvi::ViParameters& parameters, std::string dBowDir, se::SubMapConfig submapConfig = se::SubMapConfig());
  ...
  */
  bool addDepthMeasurement(const dgvi::Time &stamp,
   ...
   * \return Returns true normally. False if the previous one has not been processed yet.
   */
  virtual bool addGpsMeasurement(const dgvi::Time & stamp,
   ...
   * \return Returns true normally. False if the previous one has not been processed yet.
   */
  virtual bool addGeodeticGpsMeasurement(const dgvi::Time & stamp,
  ...
  typedef std::function<void(const dgvi::Time&, const dgvi::kinematics::Transformation&,
...
private:
  ...
  void computeLiveDepthMeasurements(const kinematics::Transformation& T_WS_live, const dgvi::Time& curTime,
  ...
  threadsafe::Queue<std::vector<dgvi::CameraMeasurement> > cameraMeasurementsReceived_;

  /// IMU measurement input queue.
  threadsafe::Queue<dgvi::ImuMeasurement, Eigen::aligned_allocator<dgvi::ImuMeasurement>>
      imuMeasurementsReceived_;

  /// Lidar measurement input queue.
  threadsafe::Queue<dgvi::LidarMeasurement , Eigen::aligned_allocator<dgvi::LidarMeasurement>>
      lidarMeasurementsReceived_;

  /// Depth-sigma measurement input queue.
  threadsafe::Queue<dgvi::CameraMeasurement, Eigen::aligned_allocator<dgvi::CameraMeasurement>>
  ...
  threadsafe::Queue<dgvi::GpsMeasurement> gpsMeasurementsReceived_;
  ...
  dgvi::ViSlamBackend estimator_;    ///< The backend estimator.
  dgvi::Frontend frontend_;      ///< The frontend.
  dgvi::Trajectory trajectory_;
  std::ofstream trajectory_debug_output_;
  std::ofstream base_frame_debug_output_;
  /// @}

  dgvi::ViParameters parameters_; ///< The parameters and settings.
  ...
  void updateVisualStationarity(const dgvi::MultiFramePtr& multiFrame);
  ...
  dgvi::kinematics::Transformation T_SD_; /// < Transformation {S} IMU <-> {D} Depth source (Lidar or Camera)
  dgvi::kinematics::Transformation T_rect_; ///< T_{D}{D_rectification}
...
#endif /* INCLUDE_DGVI_THREADEDSLAM3_HPP_ */
