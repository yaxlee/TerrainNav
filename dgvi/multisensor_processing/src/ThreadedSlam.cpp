  ...
  useAlignmentFactors_(submapConfig_.useMap2LiveFactors)
{
  ...
  //  "/Users/leuteneg/Documents/datasets/euroc/V2_03_difficult/mav0/dgvi-slam-final_map.g2o",
  ...
  if(parameters_.output.enable_submapping) {
    if(parameters_.lidar){
      ...
      if(!mapping_camera_found){
        ...
        "If you want to run without depth specify enabling_submapping: false in the dgvi_config. If you want to use" \
        "the LiDAR, please specify a lidar section in the dgvi_config. If you wanted to use depth from cameras " \
    ...
    useAlignmentFactors_ = false;
    LOG(WARNING) << "Alignment factors have been set in the se2_config but the enable_submapping in the dgvi_config is set to false." \
...
// Add a new image.
bool ThreadedSlam::addImages(const dgvi::Time & stamp,
                             ...
                             const std::map<size_t, cv::Mat> & depthImages)
{
  ...
  // assemble frame
  const size_t numCameras = parameters_.nCameraSystem.numCameras();
  std::vector<dgvi::CameraMeasurement> frames(numCameras);
...
bool ThreadedSlam::addImuMeasurement(const dgvi::Time& stamp,
                                     ...
                                     const Eigen::Vector3d& omega)
{
  ...
  dgvi::ImuMeasurement imu_measurement;
...
bool ThreadedSlam::addLidarMeasurement(const dgvi::Time &stamp,
                                       const Eigen::Vector3d &rayMeasurement)
{
  ...
  dgvi::LidarMeasurement lidarMeasurement;
...
bool ThreadedSlam::addDepthMeasurement(const dgvi::Time &stamp,
  ...
  // We want to buffer 4 depth images to give DGVI to have time to compute the states
...
bool ThreadedSlam::addGpsMeasurement(const dgvi::Time& stamp,
                                     const Eigen::Vector3d& pos,
                                     const Eigen::Vector3d& err)
{
  dgvi::GpsMeasurement  gps_measurement;
  dgvi::GpsSensorReadings gpsReading(pos, err(0), err(1), err(2));
...
bool ThreadedSlam::addGeodeticGpsMeasurement(const dgvi::Time& stamp,
                                             double lat, double lon, double height,
                                             double hAcc, double vAcc)
{

  dgvi::GpsMeasurement  gps_measurement;
  dgvi::GpsSensorReadings gpsReading(lat, lon, height, hAcc, vAcc);
  gps_measurement.timeStamp = stamp;
  gps_measurement.measurement = gpsReading;
  const int gpsQueueSize = 5000;
...
ThreadedSlam::VisualMotionStats ThreadedSlam::computeVisualMotionStats(
    const dgvi::MultiFramePtr& multiFrame,
...
void ThreadedSlam::updateVisualStationarity(const dgvi::MultiFramePtr& multiFrame)
{
...
bool ThreadedSlam::getNextFrame(MultiFramePtr &multiFrame) {
  std::vector<dgvi::CameraMeasurement> frames;
  if(!cameraMeasurementsReceived_.getCopyOfFront(&frames)) {
    return false;
  }
  const size_t numCameras = frames.size();
  multiFrame.reset(new dgvi::MultiFrame(parameters_.nCameraSystem, frames.at(0).timeStamp, 0));
  bool success = false;
  for(size_t im = 0; im < numCameras; ++im)
  {
...
void ThreadedSlam::optimisePublishMarginalise(MultiFramePtr multiFrame,
  ...
  if(optimisedGraphCallback_) {
    ...
    if(blocking_){
      // in blocked processing, we also want to be able to block the dgvi-estimator from outside
  ...
  if(parameters_.output.display_matches) {
    ...
    dgvi::ObservationVector::iterator it = visualizationDataPtr
        ->observations.begin();
    for (size_t camIndex = 0; camIndex < numCameras; ++camIndex) {
      ...
      for (size_t k = 0; k < multiFrame->numKeypoints(camIndex); ++k) {
        DGVI_ASSERT_TRUE_DBG(Exception,it != visualizationDataPtr->observations.end(),
    ...
    for (size_t i = 0; i < depthMeasurementDeque_.size(); i ++) {
        ...
        if (!trajectory_.getState(depthMeasurementDeque_[i].timeStamp, stateMeas)) {
          ...
          if (!allIds.empty()) {
            ...
            if (interpWeight > 0 && interpWeight < 1) {
              ...
              dgvi::kinematics::Transformation T_interp(interpPos, interpQuat);
      ...
      for (int x = 0; x < depthWidth; x+=10) {
        for (int y = 0; y < depthHeight; y+=10) {
          ...
          std::optional<float> occ = dgvi::interpFieldMeanOccup<se::Safe::On>(*previousSubmap_, p_M);
          std::optional<Eigen::Vector3f> gradf = dgvi::gradFieldMeanOccup<se::Safe::On>(*previousSubmap_, p_M);
      ...
      if (observedPoints.size() > submapConfig_.numSubmapFactors) {
        dgvi::downsamplePointsUncertainty(observedPoints, observedSigmas,
...
void ThreadedSlam::saveAlignedPoints(size_t id,
  ...
  std::string saveName = finalTrajectoryCsvFileName_.substr(0, finalTrajectoryCsvFileName_.find("dgvi"))
