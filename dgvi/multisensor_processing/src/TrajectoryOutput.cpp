...
dgvi::TrajectoryOutput::TrajectoryOutput(bool draw) : draw_(draw) {
  if(draw_) {
...
dgvi::TrajectoryOutput::TrajectoryOutput(const std::string &filename, bool rpg, bool draw)
    : draw_(draw) {
...
void dgvi::TrajectoryOutput::setCsvFile(const std::string &filename, bool rpg) {
  rpg_ = rpg;
  createCsvFile(filename, csvFile_, rpg);
}

void dgvi::TrajectoryOutput::setRGBCsvFile(const std::string & filename) {
  createCsvFile(filename, rgbCsvFile_, rpg_);
}

void dgvi::TrajectoryOutput::processState(
...
bool dgvi::TrajectoryOutput::addImuMeasurement(
  const dgvi::Time &stamp, const Eigen::Vector3d &alpha, const Eigen::Vector3d &omega)
{
  if(draw_) {
...
bool dgvi::TrajectoryOutput::processRGBImage(const dgvi::Time& timestamp, const cv::Mat& image) {
  if (!rgbCsvFile_) {
...
void dgvi::TrajectoryOutput::drawTopView(cv::Mat& outImg) {
  ...
  while(states_.PopNonBlocking(&graphStates)) {
    ...
    std::set<dgvi::StateId> affectedStateIds;
...
cv::Point2d dgvi::TrajectoryOutput::convertToImageCoordinates(
...
void dgvi::TrajectoryOutput::drawPath()
{
...
void dgvi::TrajectoryOutput::drawGps()
{
...
void dgvi::TrajectoryOutput::drawPathNoncausal()
{
...
bool dgvi::TrajectoryOutput::createCsvFile(
  const std::string &filename, std::fstream& stream, const bool rpg) {
  stream.open(filename.c_str(), std::ios_base::out);
  DGVI_ASSERT_TRUE(Exception, stream.good(), "couldn't create trajectory file at " << filename)
  if(rpg) {
...
bool dgvi::TrajectoryOutput::writeStateToCsv(
  std::fstream& csvFile, const dgvi::State& state, const bool rpg) {
  if (!csvFile.good()) {
    return false;
  }
