...
void ViParametersReader::readConfigFile(const std::string& filename) {
  ...
  DGVI_ASSERT_TRUE(Exception, file.isOpened(),
  ...
  if (file["camera_parameters"]["fov_scale"].isReal()) {
    ...
    if (numRectifiedCamera != 0) {
      LOG(FATAL) << "Please set fov_scale for the stereo rectification.\n"
        ...
        << "please set mapping_rectification: false in dgvi.yaml.";
    }
  }
  ...
  for (size_t i = 0; i < calibrations.size(); ++i) {
    ...
    std::shared_ptr<const kinematics::Transformation> T_SC_dgvi_ptr(
    ...
    if(strcmp(calibrations[i].cameraModel.c_str(), "eucm") == 0){
      std::shared_ptr<dgvi::cameras::EucmCamera> cam;
      cam.reset(new dgvi::cameras::EucmCamera(calibrations[i].imageDimension[0],
      ...
      viParameters_.nCameraSystem.addCamera(
              T_SC_dgvi_ptr,
              std::static_pointer_cast<const dgvi::cameras::CameraBase>(cam),
              dgvi::cameras::NCameraSystem::NoDistortion, true,
      ...
      std::shared_ptr<dgvi::cameras::PinholeCamera<dgvi::cameras::EquidistantDistortion>> cam;
      cam.reset(new dgvi::cameras::PinholeCamera<
                  dgvi::cameras::EquidistantDistortion>(
  ...
  if(file["lidar"].isMap()){
    viParameters_.lidar = dgvi::LidarParameters();
  ...
  if(file["gps_parameters"].isMap()){
    viParameters_.gps = dgvi::GpsParameters();
  ...
  if(file["dem_parameters"].isMap()){
    viParameters_.dem = dgvi::DemParameters();
...
void ViParametersReader::parseEntry(const cv::FileNode& file, std::string name, int& readValue) {
  DGVI_ASSERT_TRUE(Exception, file[name].isInt(),
...
void ViParametersReader::parseEntry(const cv::FileNode &file,
  ...
  DGVI_ASSERT_TRUE(Exception, file[name].isReal(),
...
void ViParametersReader::parseEntry(const cv::FileNode& file, std::string name, bool& readValue) {
  DGVI_ASSERT_TRUE(Exception, file[name].isInt() || file[name].isString(),
  ...
  if (file[name].isString()) {
    ...
    DGVI_THROW(Exception, "Boolean with uninterpretable value " << str)
  }
  return;
}
...
void ViParametersReader::parseEntry(const cv::FileNode &file, std::string name,
  ...
  DGVI_ASSERT_TRUE(Exception, T.isSeq(),
...
void ViParametersReader::parseEntry(const cv::FileNode& file, std::string name,
  ...
  cv::FileNode T = file[name];
  DGVI_ASSERT_TRUE(Exception, T.isString(),
    ...
    for (cv::FileNodeIterator it = cameraNode.begin();
      ...
      if((std::string)((*it)["cam_model"]) == "pinhole"){
        ...
        T_SC << T_SC_node[0], T_SC_node[1], T_SC_node[2], T_SC_node[3],
                ...
                T_SC_node[8], T_SC_node[9], T_SC_node[10], T_SC_node[11],
                T_SC_node[12], T_SC_node[13], T_SC_node[14], T_SC_node[15];
        calib.T_SC = dgvi::kinematics::Transformation(T_SC);
...
bool ViParametersReader::getLiDARCalibration(const cv::FileNode& calibrationNode, dgvi::LidarParameters& lidarParameters){
  ...
  lidarParameters.T_SL = dgvi::kinematics::Transformation(T_SL);
...
bool ViParametersReader::getGpsCalibration(const cv::FileNode& calibrationNode, dgvi::GpsParameters& gpsParameters){
...
bool ViParametersReader::computeRectifyMap(CameraCalibration& leftCalibration,
  ...
  if (leftCalibration.cameraType.depthType.needRectify && rightCalibration.cameraType.depthType.needRectify &&
    ...
    std::shared_ptr<const kinematics::Transformation> T_SCl_dgvi_ptr(
    ...
    viParameters_.nCameraSystem.addRectifyCamera(stereo_indices[0], T_SCl_dgvi_ptr, rectify_caml);

    // Rectified right camera
    kinematics::Transformation T_SCr = rightCalibration.T_SC * T2_rect_unrect.inverse();
        std::shared_ptr<const kinematics::Transformation> T_SCr_dgvi_ptr(
    ...
    viParameters_.nCameraSystem.addRectifyCamera(stereo_indices[1], T_SCr_dgvi_ptr, rectify_camr);
    return true;
  } else {
    return false;
  }
