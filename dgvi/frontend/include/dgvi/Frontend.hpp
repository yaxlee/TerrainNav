...
#ifndef INCLUDE_DGVI_FRONTEND_HPP_
#define INCLUDE_DGVI_FRONTEND_HPP_

#include <mutex>

#include <dgvi/Component.hpp>
#include <dgvi/ViFrontendInterface.hpp>
...
class Frontend : public ViFrontendInterface {
 public:
  ...
  virtual bool detectAndDescribe(size_t cameraIndex,
                                 std::shared_ptr<dgvi::MultiFrame> frameOut,
  ...
  virtual bool dataAssociationAndInitialization(
      ...
      std::shared_ptr<dgvi::MultiFrame> framesInOut, bool kfPrior, bool* asKeyframe) override final;
  ...
  /// @brief Set rectangular regions excluded from feature detection (e.g. vehicle body parts).
  void setDetectionMaskRects(const std::vector<cv::Rect>& rects) {
...
private:
  ...
  void matchStereo(Estimator& estimator,
                   std::shared_ptr<dgvi::MultiFrame> multiFrame,
                   const dgvi::ViParameters& params,
                   bool asKeyframe);
  ...
  int runRansac2d2d(Estimator& estimator,
                    const dgvi::ViParameters& params, uint64_t currentFrameId,
   ...
   */
  template<class CAMERA_GEOMETRY>
  int matchMotionStereo(Estimator &estimator, const dgvi::ViParameters &params,
...
#endif // INCLUDE_DGVI_FRONTEND_HPP_
