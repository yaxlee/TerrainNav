...
TEST(dgviTestSuite, GpsErrorSynchronous){
    ...
    dgvi::kinematics::Transformation T_GW; // GPS to workd
    ...
    dgvi::ceres::PoseParameterBlock poseParameterBlock(T_GW_init,1,dgvi::Time(0));
    problem.AddParameterBlock(poseParameterBlock.parameters(), dgvi::ceres::PoseParameterBlock::Dimension);
    problem.SetParameterBlockVariable(poseParameterBlock.parameters()); // Optimize this!
    // Set local parametrization
    ::ceres::LocalParameterization* poseLocalParameterization = new dgvi::ceres::PoseLocalParameterization;
    ...
    for(int i = 0; i < N; i++){
        ...
        // disturb robot pose
        dgvi::kinematics::Transformation T_dist_WS;
        T_dist_WS.setRandom(0.5, M_PI);
        dgvi::kinematics::Transformation T_WS_init(T_dist_WS*T_WS);

        // True GPS Signal
        Eigen::Vector3d position_G = T_GW.C() * (T_WS.r() + T_WS.C()*r_SA)+ T_GW.r();

        // add a parameter block for robots position
        dgvi::ceres::PoseParameterBlock* robotPoseParameterBlock = new dgvi::ceres::PoseParameterBlock(T_WS_init,1,dgvi::Time(0));
        problem.AddParameterBlock(robotPoseParameterBlock->parameters(), dgvi::ceres::PoseParameterBlock::Dimension);
        ...
        ::ceres::CostFunction* cost_function = new dgvi::ceres::GpsErrorSynchronous(1,position_G,information, gpsParameters);
        ...
        dgvi::ceres::PoseParameterBlock* refParameterBlock = new dgvi::ceres::PoseParameterBlock(T_WS,1,dgvi::Time(0));
        problem.AddParameterBlock(refParameterBlock->parameters(), dgvi::ceres::PoseParameterBlock::Dimension);
        ...
        ::ceres::CostFunction* cost_function_rel = new dgvi::ceres::RelativePoseError(information_rel,T_rel);
