...
TEST(dgviTestSuite, ImuError){
  ...
  const double dt=1.0/double(imuRate); // time discretization
	...
	// start
	dgvi::kinematics::Transformation T_WS_0;
	dgvi::SpeedAndBias speedAndBias_0;
	dgvi::Time t_0;

	// end
	dgvi::kinematics::Transformation T_WS_1;
	dgvi::SpeedAndBias speedAndBias_1;
	dgvi::Time t_1;
  ...
  for(size_t i=0; i<size_t(duration*imuRate); ++i){
    ...
    imuMeasurements.push_back(
          dgvi::ImuMeasurement(dgvi::Time(time),dgvi::ImuSensorReadings(gyr,acc)));
	}

	// create the pose parameter blocks
	dgvi::kinematics::Transformation T_disturb;
	T_disturb.setRandom(1,0.02);
	dgvi::kinematics::Transformation T_WS_1_disturbed=T_WS_1*T_disturb; //
	dgvi::ceres::PoseParameterBlock poseParameterBlock_0(T_WS_0,0,t_0); // ground truth
	dgvi::ceres::PoseParameterBlock poseParameterBlock_1(T_WS_1_disturbed,2,t_1); // disturbed...
  ...
  problem.AddParameterBlock(poseParameterBlock_1.parameters(),
	...
	dgvi::ceres::SpeedAndBiasParameterBlock speedAndBiasParameterBlock_0(speedAndBias_0,1,t_0);
	dgvi::ceres::SpeedAndBiasParameterBlock speedAndBiasParameterBlock_1(speedAndBias_1,3,t_1);
  problem.AddParameterBlock(speedAndBiasParameterBlock_0.parameters(),
                            dgvi::ceres::SpeedAndBiasParameterBlock::Dimension);
  ...
  ::ceres::Manifold* poseManifold = new dgvi::ceres::PoseManifold;
  ...
  dgvi::ceres::ImuError* cost_function_imu =
      new dgvi::ceres::ImuError(imuMeasurements, imuParameters,t_0, t_1);
  ...
  ::ceres::CostFunction* cost_function_pose =
      new dgvi::ceres::PoseError(T_WS_0, 1e-12, 1e-4); // pose prior...
  ...
  ::ceres::CostFunction* cost_function_speedAndBias =
      new dgvi::ceres::SpeedAndBiasError(speedAndBias_0, 1e-12, 1e-12, 1e-12); // speed/bias prior
  ...
  DGVI_ASSERT_TRUE(Exception, summary.final_cost<1e-2,"cost not reducible")
