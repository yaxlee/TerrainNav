                     ...
                     kinematics::Transformation& T_GW_refined)
{
  ...
  dgvi::ceres::PoseParameterBlock gpsParameterBlock(T_GW_init,0,dgvi::Time(0));
  auto* gpsExtrinsicLocalParametrisation = new dgvi::ceres::PoseManifold4d();
  gpsParameterBlock.setLocalParameterizationPtr(gpsExtrinsicLocalParametrisation);
  problem.AddParameterBlock(gpsParameterBlock.parameters(), dgvi::ceres::PoseParameterBlock::Dimension, gpsExtrinsicLocalParametrisation);
    ...
    const cameras::NCameraSystem & nCameraSystem)
{
  ...
  DGVI_ASSERT_TRUE_DBG(Exception, states_.size()==0, "states added before...")
  StateId id(1);
  ...
  if (imuParametersVec_.at(0).use) {
    DGVI_ASSERT_TRUE_DBG(Exception, imuMeasurements.size() > 0, "no IMU measurements passed")

    // acceleration vector
    Eigen::Vector3d acc_B = Eigen::Vector3d::Zero();
    for (dgvi::ImuMeasurementDeque::const_iterator it = imuMeasurements.begin();
                                     ...
                                     const ImuMeasurementDeque &imuMeasurements, bool isKeyframe)
{
  ...
  //DGVI_ASSERT_TRUE(
  //  Exception, dgvi::ceres::jacobiansCorrect(problem_.get(), imuLink.residualBlockId),
...
{
  DGVI_ASSERT_TRUE_DBG(Exception, other.states_.count(stateId), "stateId not found")
  ...
  if(otherState.previousImuLink.errorTerm) {
    ...
    DGVI_ASSERT_TRUE_DBG(Exception, otherIter != other.states_.begin(), "no previous state")
    otherIter--;
    ...
    DGVI_ASSERT_TRUE_DBG(Exception, otherIter->first == states_.rbegin()->first,
                      "different previous states")
    DGVI_ASSERT_TRUE_DBG(Exception, t_0 == previousState.timestamp, "inconsistent previous times")
    ImuLink imuLink;
...
bool ViGraph::addLandmark(LandmarkId landmarkId, const Eigen::Vector4d &homogeneousPoint,
                           bool initialised)
{
  DGVI_ASSERT_TRUE_DBG(Exception, landmarkId.isInitialised(), "landmark ID invalid")
  DGVI_ASSERT_TRUE_DBG(Exception, landmarks_.count(landmarkId) == 0, "landmark already exists")
  Landmark landmark;
...
LandmarkId ViGraph::addLandmark(const Eigen::Vector4d &homogeneousPoint, bool initialised)
{
  ...
  problem_->AddParameterBlock(landmark.hPoint->parameters(), 4,
                              &homogeneousPointManifold_);
  landmark.hPoint->setLocalParameterizationPtr(&homogeneousPointManifold_);
  DGVI_ASSERT_TRUE_DBG(Exception, landmarks_.count(landmarkId) == 0, "Bug: landmark not added")
  landmarks_[landmarkId] = landmark;
  return landmarkId;
}
...
{
  DGVI_ASSERT_TRUE_DBG(Exception, landmarks_.count(landmarkId), "landmark does not exists")
  ...
  for(auto state : states_) {
    for(auto obs : state.second.observations) {
      DGVI_ASSERT_TRUE(Exception,
                        landmarks_.at(obs.second.landmarkId).observations.count(obs.first),
                        "observations check failed")
      DGVI_ASSERT_TRUE(Exception, observations_.count(obs.first), "observations check failed")
    }
  }
...
{
  DGVI_ASSERT_TRUE_DBG(Exception, observations_.count(keypointId), "observation does not exists")
  ...
  DGVI_ASSERT_TRUE_DBG(Exception, landmarks_.count(observation.landmarkId),
                        "landmark does not exists")
  Landmark& landmark = landmarks_.at(observation.landmarkId);
  DGVI_ASSERT_TRUE_DBG(Exception, landmark.observations.count(keypointId),
                    "observation does not exists")
  landmark.observations.erase(keypointId);
  StateId stateId(keypointId.frameId);
  DGVI_ASSERT_TRUE_DBG(Exception, states_.count(stateId),
                    "state does not exists")
  State& state = states_.at(stateId);
  DGVI_ASSERT_TRUE_DBG(Exception, state.observations.count(keypointId),
                    "observation does not exists")
...
{
  DGVI_ASSERT_TRUE(Exception, covisibilitiesComputed_, "covisibilities not yet computed")
  if(pose_i==pose_j) {
    return 0;
  }
...
{
  DGVI_ASSERT_TRUE_DBG(Exception, states_.count(poseId0), "stateId not found")
  DGVI_ASSERT_TRUE_DBG(Exception, states_.count(poseId1), "stateId not found")
  State & state0 = states_.at(poseId0);
  State & state1 = states_.at(poseId1);
  DGVI_ASSERT_TRUE_DBG(Exception, state0.relativePoseLinks.count(poseId1)==0,
                    "relative pose error already exists")
  DGVI_ASSERT_TRUE_DBG(Exception, state1.relativePoseLinks.count(poseId0)==0,
                    "relative pose error already exists")
...
{
  DGVI_ASSERT_TRUE_DBG(Exception, states_.count(poseId0),
                        "stateId " << poseId0.value() << " not found")
  DGVI_ASSERT_TRUE_DBG(Exception, states_.count(poseId1),
                        "stateId " << poseId1.value() << "not found")
  State & state0 = states_.at(poseId0);
  State & state1 = states_.at(poseId1);
  DGVI_ASSERT_TRUE_DBG(Exception, state0.relativePoseLinks.count(poseId1)==1,
                    "relative pose error does not exists")
  DGVI_ASSERT_TRUE_DBG(Exception, state1.relativePoseLinks.count(poseId0)==1,
                    "relative pose error does not exists")
...
bool ViGraph::needsFullGpsAlignment(StateId& gpsLossId, StateId& gpsAlignId, dgvi::kinematics::Transformation& T_GW_new){
...
bool ViGraph::needsPosGpsAlignment(StateId& gpsLossId, StateId& gpsAlignId, Eigen::Vector3d& posError){
    ...
    if(needsPositionAlignment_){
        ...
        dgvi::ceres::GpsErrorAsynchronous test(lastMeasurement.measurement.position, lastMeasurement.measurement.covariances.inverse(),
        ...
        dgvi::kinematics::Transformation T_GW_original = states_.begin()->second.T_GW->estimate();
...
bool ViGraph::applyBoundedGpsWindowCorrection(const Eigen::Vector3d& correction_W,
  ...
  for(auto& stateEntry : states_) {
    ...
    const dgvi::kinematics::Transformation T_WS = state.pose->estimate();
    const dgvi::kinematics::Transformation T_WS_shifted(T_WS.r() + correction_W,
                                                         T_WS.q());
...
bool ViGraph::maybeApplyBoundedGpsRecovery(StateId poseId,
  ...
  dgvi::kinematics::Transformation T_WS_prop;
  gpsError.applyPreInt(state.pose->estimate(), state.speedAndBias->estimate(), T_WS_prop);

  const dgvi::kinematics::Transformation T_GW_curr = T_GW();
...
bool ViGraph::addGpsMeasurement(StateId poseId, GpsMeasurement &gpsMeas, const ImuMeasurementDeque &imuMeasurements){
    ...
    DGVI_ASSERT_TRUE(Exception, states_.count(poseId), "stateId " << poseId.value() << " not found")
    DGVI_ASSERT_TRUE(Exception, (gpsMeas.timeStamp >= states_.at(poseId).timestamp), "GPS measurement too old to add to state" )
    ...
    //DGVI_ASSERT_TRUE(Exception, (imuMeasurements.front().timeStamp <= states_.at(poseId).timestamp), "IMU Measurements for adding GPS error are not old enough");
    DGVI_ASSERT_TRUE(Exception, (imuMeasurements.back().timeStamp >= gpsMeas.timeStamp), "IMU measurements do not cover GPS measurement");
        ...
        if(dt >= minDt && dt <= maxDt) {
          ...
          if(gpsParameters.gpsMaxSpeed <= 0.0 || gpsSpeed <= gpsParameters.gpsMaxSpeed) {
            ...
            if(!velocityPriorState) {
              ...
              dgvi::SpeedAndBias speedAndBiasMeasurement =
              ...
              dgvi::ceres::SpeedAndBiasError::information_t information;
              information.setZero();
...
bool ViGraph::bootstrapTGWFromTwoPoints(kinematics::Transformation& T_GW_bootstrap,
  ...
  dgvi::kinematics::Transformation T_WS1_prop, T_WS2_prop;
  const dgvi::SpeedAndBias sb1 = states_.at(sid1).speedAndBias->estimate();
  const dgvi::SpeedAndBias sb2 = states_.at(sid2).speedAndBias->estimate();
...
bool ViGraph::checkForGpsInit(dgvi::kinematics::Transformation& T_GW, std::set<StateId> consideredStates, double* yaw_error) {
  ...
  if(gpsParametersVec_.back().robustGpsInit){
    ...
    for(; gpsStateIter != consideredStates.end(); gpsStateIter++){
      ...
      dgvi::kinematics::Transformation T_WS_state = states_.at(gpsState).pose->estimate();
      dgvi::SpeedAndBias sb_state = states_.at(gpsState).speedAndBias->estimate();
  ...
  for(size_t i = 0; i < worldPoints.size(); ++i){
    ...
    Eigen::Matrix<double,3,3> tmp = dgvi::kinematics::crossMx(T_GW.C()*pt);
...
int ViGraph::checkValidGpsMeasurements(GpsMeasurementDeque& inputGpsMeasurementDeque, GpsMeasurementDeque& gpsMeasurementDeque){
  ...
  for(auto rIterMeas = inputGpsMeasurementDeque.rbegin(); rIterMeas != inputGpsMeasurementDeque.rend() ; rIterMeas++){
      ...
      if ((gpsStatus_ == gpsStatus::Initialised && !needsReInit)){
        ...
        dgvi::kinematics::Transformation T_WS_state = states_.at(sid).pose->estimate();
        dgvi::kinematics::Transformation T_GW_curr_est = states_.at(sid).T_GW->estimate();
        if(gpsParametersVec_.back().type == "geodetic" || gpsParametersVec_.back().type == "geodetic-leica"){
          double x=0,y=0,z=0;
...
bool ViGraph::addGpsMeasurements(GpsMeasurementDeque& gpsMeasurementDeque, ImuMeasurementDeque& imuMeasurementDeque, std::deque<StateId>* sids){
  ...
  DGVI_ASSERT_TRUE(Exception, (imuMeasurementDeque.front().timeStamp <= gpsMeasurementDeque.front().timeStamp), "IMU measurements not old enough for gps measurement");
  DGVI_ASSERT_TRUE(Exception, (imuMeasurementDeque.back().timeStamp >= gpsMeasurementDeque.back().timeStamp), "GPS measurement not covered by IMU measurements");
...
bool ViGraph::initializationStrategy(kinematics::Transformation& T_GW_est) {
  ...
  dgvi::kinematics::Transformation T_GW_init;
...
void ViGraph::getLandmarkHessian(LandmarkId lmId, Eigen::Matrix3d& H) {
  DGVI_ASSERT_TRUE_DBG(Exception,landmarks_.count(lmId) ,"parameter block not in map.");
...
bool ViGraph::addSubmapAlignmentConstraints(const SupereightMapType* submap_ptr,
  ...
  for(uint64_t i = 0; i < pointCloud.size(); i++){
    ...
    newSubmapAlignmentFactor.errorTerm.reset(new dgvi::ceres::SubmapIcpError(*submap_ptr, pt.cast<double>(), sigma_i));
    ...
    if(debugLidarResiduals_){
      ...
      jacobians[0] = J0.data();
      jacobians[1] = J1.data();
      static_cast<dgvi::ceres::SubmapIcpError *>(newSubmapAlignmentFactor.errorTerm.get())->EvaluateWithMinimalJacobians(parameter_blocks.data(),
      ...
      auto nonZeroFieldAndGradient = static_cast<dgvi::ceres::SubmapIcpError*>(newSubmapAlignmentFactor.errorTerm.get())->nonZeroFieldAndGradient(parameter_blocks.data(), fieldVal, gradNorm);
...
{
  DGVI_ASSERT_TRUE_DBG(Exception, landmarks_.count(id), "landmark does not exists")
...
{
  DGVI_ASSERT_TRUE_DBG(Exception, states_.count(id), "state does not exists")
...
{
  DGVI_ASSERT_TRUE(Exception, states_.count(id), "state does not exists")
...
{
  DGVI_ASSERT_TRUE_DBG(Exception, states_.count(id), "state does not exists")
  DGVI_ASSERT_TRUE_DBG(Exception, states_.at(id).extrinsics.at(camIdx), "extrinsics do not exists")
...
{
  DGVI_ASSERT_TRUE_DBG(Exception, !states_.empty(), "no states exist")
  return states_.rbegin()->first;
}

StateId ViGraph::stateIdByAge(size_t age) const
{
  DGVI_ASSERT_TRUE_DBG(Exception, states_.size()>age, "too few states exist")
  auto iter = states_.rbegin();
...
//  covariance_blocks.push_back(std::make_pair(states_.at(dgvi::StateId(1)).T_GW->parameters(),states_.at(dgvi::StateId(1)).T_GW->parameters()));
...
//    covariance.GetCovarianceBlockInTangentSpace(states_.at(dgvi::StateId(1)).T_GW->parameters() , states_.at(dgvi::StateId(1)).T_GW->parameters() ,covariance_gg);
...
bool ViGraph::setOptimisationTimeLimit(double timeLimit, int minIterations)
{
  if (ceresCallback_ != nullptr) {
    ...
    ceresCallback_ = std::unique_ptr<dgvi::ceres::CeresIterationCallback>(
          new dgvi::ceres::CeresIterationCallback(timeLimit, minIterations));
