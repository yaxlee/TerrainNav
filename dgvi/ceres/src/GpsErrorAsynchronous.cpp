...
#include <dgvi/ceres/ImuError.hpp> // required for propagation
...
bool GpsErrorAsynchronous::EvaluateWithMinimalJacobians(double const* const * parameters,
    ...
    dgvi::kinematics::Transformation T_WS_tk(r_WS, q_WS);
    ...
    dgvi::kinematics::Transformation T_WS_tg;
    T_WS_tg.set(r_WS + speedAndBiases.head<3>()*Delta_t - 0.5*g_W*Delta_t*Delta_t
                ...
                T_WS_tk.q()*Delta_q_*dgvi::kinematics::deltaQ(-dalpha_db_g_*Delta_b.head<3>()));

    // assign Jacobian w.r.t. x0
    Eigen::Matrix<double,15,15> Jprop =
      Eigen::Matrix<double,15,15>::Identity(); // holds for d/db_g, d/db_a

    Jprop.block<3,3>(0,3) = -dgvi::kinematics::crossMx(C_WS_tk*acc_doubleintegral_);
    ...
    Jprop.block<3,3>(6,3) = -dgvi::kinematics::crossMx(C_WS_tk*acc_integral_);
    ...
    if(useImuCovHere){
        ...
        T.topLeftCorner<3,3>() = T_WS_tk.C();
        T.block<3,3>(3,3) = T_WS_tk.C();
        T.block<3,3>(6,6) = T_WS_tk.C();
        ...
        Jws.block<3,3>(0,3) = -C_GW * dgvi::kinematics::crossMx(T_WS_tk.C()*r_SA);
    ...
    if(jacobians!=NULL){
        ...
        if(jacobians[0]!=NULL){ // w.r.t. robot pose at tk
            ...
            J0_minimal_tg.topRightCorner<3,3>() = C_GW * dgvi::kinematics::crossMx(T_WS_tg.C()*r_SA);
        ...
        if(jacobians[1]!=NULL){
            ...
            J1_minimal_tg.topRightCorner<3,3>() = C_GW * dgvi::kinematics::crossMx(T_WS_tg.C()*r_SA);
        ...
        if(jacobians[2]!=NULL){ // w.r.t. GPS/world trafo
            ...
            J2_minimal.topRightCorner<3,3>() = dgvi::kinematics::crossMx(C_GW*(T_WS_tg.r()+T_WS_tg.C()*r_SA));
...
int GpsErrorAsynchronous::redoPreintegration(const dgvi::kinematics::Transformation& /*T_WS*/,
  ...
  for (dgvi::ImuMeasurementDeque::const_iterator it = imuMeasurements_.begin();
    ...
    const double sinc_theta_half = dgvi::kinematics::sinc(theta_half);
