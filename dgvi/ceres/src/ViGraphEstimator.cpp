...
{
  DGVI_ASSERT_TRUE_DBG(Exception, states_.count(stateId), "State ID not found")
...
bool ViGraphEstimator::eliminateStateByImuMerge(StateId stateId, StateId refId)
{
  ...
  DGVI_ASSERT_TRUE_DBG(Exception, states_.at(stateId).twoPoseConstLinks.size()==0,
                        "two pose const links present at state ID=" << stateId.value())
  DGVI_ASSERT_TRUE_DBG(Exception, states_.at(stateId).twoPoseLinks.size()==0,
                        "two pose links present at state ID=" << stateId.value())
  auto iterPrevious = states_.find(stateId);
  DGVI_ASSERT_TRUE_DBG(Exception, iterPrevious!=states_.begin(), "cannot eliminate first state")
  iterPrevious--;
  auto iterNext = states_.find(stateId);
  iterNext++;
  DGVI_ASSERT_TRUE_DBG(Exception, iterNext!=states_.end(), "cannot eliminate last state")
...
//      dgvi::ImuMeasurementDeque currentOwnedMeasurements = facIter->errorTerm.get()->imuMeasurements();
//      dgvi::ImuMeasurementDeque mergedMeasurements = previousImuLink.errorTerm.get()->imuMeasurements();
...
//      dgvi::Time tg = facIter->errorTerm.get()->tg();
...
{
  DGVI_ASSERT_TRUE_DBG(Exception, landmarks_.count(fromId), "Landmark ID not found")
  DGVI_ASSERT_TRUE_DBG(Exception, landmarks_.count(intoId), "Landmark ID not found")
  ...
  DGVI_ASSERT_TRUE_DBG(Exception, fromLandmark.observations.size() == 0, "observations present")
...
{
  DGVI_ASSERT_TRUE_DBG(Exception, states_.count(stateId), "State ID not found")
  DGVI_ASSERT_TRUE(Exception, !removeInCeres, "not supported")
  for(auto iter = states_.find(stateId); ; --iter) {
    ...
    std::vector<KeypointIdentifier> * removedObservations)
{
  ...
  for(auto edge : edgesToCreate) {
    ...
    DGVI_ASSERT_TRUE_DBG(Exception, referenceState.isKeyframe, "state must be KF")
    DGVI_ASSERT_TRUE_DBG(Exception, otherState.isKeyframe, "state must be KF")
    ...
    std::vector<ViGraphEstimator::PoseGraphEdge> &poseGraphEdges)
{
  ...
  DGVI_ASSERT_TRUE_DBG(Exception, !mstEdges_.empty() ,"MST not created properly")
...
bool ViGraphEstimator::removeTwoPoseConstLinks(StateId convertStateId) {
  ...
  for(auto iter=poseGraphErrorTerms.begin(); iter!=poseGraphErrorTerms.end(); ++iter) {
    ...
    DGVI_ASSERT_TRUE_DBG(Exception, states_.at(stateId).twoPoseConstLinks.count(convertStateId),
...
{
  // remove links in connected states
  DGVI_ASSERT_TRUE_DBG(Exception, states_.at(pose_i).twoPoseConstLinks.count(pose_j),
                        "two pose link does not exist")
  DGVI_ASSERT_TRUE_DBG(Exception, states_.at(pose_j).twoPoseConstLinks.count(pose_i),
                        "two pose link does not exist")
...
int ViGraphEstimator::buildMst(const std::set<StateId> &states, bool keyframesOnly)
{
  ...
  DGVI_ASSERT_TRUE(Exception, success0, "covisibilities could not be computed")
...
void ViGraphEstimator::checkSynchedStates(const ViGraphEstimator & other) const
{
  for(auto iter = other.states_.begin(); iter != other.states_.end(); ++iter) {
    DGVI_ASSERT_TRUE(Exception, states_.count(iter->first) != 0, "inconsistent states")
    const State & otherState = iter->second;
    const State & thisState = states_.at(iter->first);
    DGVI_ASSERT_TRUE(Exception, thisState.isKeyframe == otherState.isKeyframe,
