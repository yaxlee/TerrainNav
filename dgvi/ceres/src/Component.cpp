...
bool Component::load(const std::string &path)
{
  ...
  DGVI_ASSERT_TRUE(Exception, file, "Could not open " << path << " for reading.");
  DGVI_ASSERT_TRUE(Exception, !fullGraph_, "Graph not empty.");
    ...
    if ((type == "VERTEX_SE3:QUAT_TIME") || (type == "VERTEX_R3:VEL")
      ...
      while (std::getline(file, line)) {
        ...
        if (type == "FRAME:KEYPOINT") {
          ...
          DGVI_ASSERT_TRUE(Exception, stateId64_2 == stateId64, "mismatching keypoint stateId")
          DGVI_ASSERT_TRUE(Exception, cameraIdx == i_2, "mismatching keypoint stateId")
      ...
      if (!multiFrames_.count(stateId)) {
        DGVI_THROW(Exception, "Observation to non-existant multi-frame")
      } else {
      ...
      file.close();
      DGVI_THROW(Exception, "Error parsing " << path << ", unknown type tag " << type)
      return false;
    }
  }
...
bool Component::save(const std::string &path)
{
  ...
  DGVI_ASSERT_TRUE(Exception, file, "Could not open " << path << " for writing.");
  ...
  for (auto iter = fullGraph_->states_.begin(); iter != fullGraph_->states_.end(); ++iter) {
    ...
    if (state.previousImuLink.errorTerm) {
      ...
      if(imuError) {
        ...
        const dgvi::ImuMeasurementDeque imuMeasurements = imuError->imuMeasurements();
