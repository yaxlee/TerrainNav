...
ImuError::ImuError(const dgvi::ImuMeasurementDeque & imuMeasurements,
                   const dgvi::ImuParameters & imuParameters,
                   const dgvi::Time& t_0, const dgvi::Time& t_1) {
...
int ImuError::append(const dgvi::kinematics::Transformation& /*T_WS*/,
                     const dgvi::SpeedAndBias & speedAndBiases,
                     const dgvi::ImuMeasurementDeque & imuMeasurements, const dgvi::Time& t_1)
{
  // sanity check for time overlap and end time consistency
  DGVI_ASSERT_TRUE_DBG(Exception, t1_ >= imuMeasurements.front().timeStamp,
                        "First IMU measurement included in ImuError is not old enough!")
  DGVI_ASSERT_TRUE_DBG(Exception, t_1 <= imuMeasurements.back().timeStamp,
                        "Last IMU measurement included in ImuError is not new enough!")

  // merge IMU deque
  dgvi::ImuMeasurementDeque::const_iterator iter = imuMeasurements.begin();
  ...
  for (dgvi::ImuMeasurementDeque::const_iterator it = imuMeasurements.begin();
    ...
    // Jacobian parts
    dalpha_db_g_ += C_1 * dgvi::kinematics::rightJacobian(omega_S_true * dt) * dt;
    const Eigen::Matrix3d cross_1 = dq.inverse().toRotationMatrix() * cross_
        + dgvi::kinematics::rightJacobian(omega_S_true * dt) * dt;
    const Eigen::Matrix3d acc_S_x = dgvi::kinematics::crossMx(acc_S_true);
    ...
    // transform
    F_delta.block<3, 3>(0, 3) = -dgvi::kinematics::crossMx(
    ...
    F_delta.block<3, 3>(6, 3) = -dgvi::kinematics::crossMx(
        0.5 * (C + C_1) * acc_S_true * dt);
...
int ImuError::redoPreintegration(const dgvi::kinematics::Transformation& /*T_WS*/,
  ...
  // sanity check:
  DGVI_ASSERT_TRUE_DBG(Exception, imuMeasurements_.front().timeStamp<=time,
                    imuMeasurements_.front().timeStamp << "<=" <<time)
  if (!(imuMeasurements_.back().timeStamp >= end)) {
    DGVI_THROW_DBG(Exception, imuMeasurements_.back().timeStamp << " >= " << end)
    return -1;  // nothing to do...
  }
  ...
  bool hasStarted = false;
  int i = 0;
  for (dgvi::ImuMeasurementDeque::const_iterator it = imuMeasurements_.begin();
...
// Propagates pose, speeds and biases with given IMU measurements.
int ImuError::propagation(const dgvi::ImuMeasurementDeque & imuMeasurements,
                          const dgvi::ImuParameters & imuParams,
                          dgvi::kinematics::Transformation& T_WS,
                          dgvi::SpeedAndBias & speedAndBiases,
                          const dgvi::Time & t_start,
                          const dgvi::Time & t_end, covariance_t* covariance,
                          jacobian_t* jacobian) {

  // now the propagation
  dgvi::Time time = t_start;
  dgvi::Time end = t_end;

  // sanity check:
  DGVI_ASSERT_TRUE(Exception, imuMeasurements.front().timeStamp<=time,
  ...
  for (dgvi::ImuMeasurementDeque::const_iterator it = imuMeasurements.begin();
    ...
    if (covariance) {
      ...
      F_delta.block<3,3>(3,9) = -dt*C_1;
      F_delta.block<3,3>(6,3) = -dgvi::kinematics::crossMx(0.5*(C + C_1)*acc_S_true*dt);
  ...
  if (jacobian) {
    ...
    F.block<3,3>(0,3) = -dgvi::kinematics::crossMx(C_WS_0*acc_doubleintegral);
    ...
    F.block<3,3>(6,3) = -dgvi::kinematics::crossMx(C_WS_0*acc_integral);
...
bool ImuError::EvaluateWithMinimalJacobians(double const* const * parameters,
  ...
  const dgvi::kinematics::Transformation T_WS_1(
  ...
  // get speed and bias
  dgvi::SpeedAndBias speedAndBiases_0;
  dgvi::SpeedAndBias speedAndBiases_1;
  for (int i = 0; i < 9; ++i) {
  ...
  // actual propagation output:
  {
    ...
    F0.block<3,3>(0,3) = C_S0_W * dgvi::kinematics::crossMx(delta_p_est_W);
    F0.block<3,3>(0,6) = C_S0_W * Eigen::Matrix3d::Identity()*Delta_t;
    F0.block<3,3>(0,9) = dp_db_g_;
    F0.block<3,3>(0,12) = -C_doubleintegral_;
    F0.block<3,3>(3,3) = (dgvi::kinematics::plus(Dq*T_WS_1.q().inverse()) *
        dgvi::kinematics::oplus(T_WS_0.q())).topLeftCorner<3,3>();
    F0.block<3,3>(3,9) = (dgvi::kinematics::oplus(T_WS_1.q().inverse()*T_WS_0.q())*
        dgvi::kinematics::oplus(Dq)).topLeftCorner<3,3>()*(-dalpha_db_g_);
    F0.block<3,3>(6,3) = C_S0_W * dgvi::kinematics::crossMx(delta_v_est_W);
    ...
    F1.block<3,3>(3,3) = -(dgvi::kinematics::plus(Dq) *
        dgvi::kinematics::oplus(T_WS_0.q()) *
        dgvi::kinematics::plus(T_WS_1.q().inverse())).topLeftCorner<3,3>();
