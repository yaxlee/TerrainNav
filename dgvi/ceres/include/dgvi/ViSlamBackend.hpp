...
/**
 * @file dgvi/ViSlamBackend.hpp
 * @brief Header file for the Estimator class. This does all the backend work.
 * @author Stefan Leutenegger
 */

#ifndef INCLUDE_DGVI_VISLAMBACKEND_HPP_
#define INCLUDE_DGVI_VISLAMBACKEND_HPP_
...
class ViSlamBackend //: public VioBackendInterface
{
 public:
   ...
   */
  int addCamera(
      const dgvi::CameraParameters & cameraParameters);
   ...
   */
  bool addStates(dgvi::MultiFramePtr multiFrame,
  ...
  bool addObservation(LandmarkId landmarkId, StateId stateId,
    ...
    DGVI_ASSERT_TRUE_DBG(Exception, multiFrames_.count(stateId), "frame does noe exist")
    ...
    DGVI_ASSERT_TRUE_DBG(Exception, !auxiliaryStates_.at(stateId).isPoseGraphFrame,
   ...
   */
  dgvi::MultiFramePtr multiFrame(StateId stateId) const {
  ...
  bool isPoseGraphFrame(StateId id) const {
    //DGVI_CHECK_MAP(auxiliaryStates_,id);
   ...
   */
  dgvi::Time timestamp(StateId id) const;
  ...
  /// \return True on success
  bool attemptFullGpsAlignment(StateId gpsLossId , StateId gpsReturnId, const dgvi::kinematics::Transformation& T_GW_new);
...
#endif /* INCLUDE_DGVI_VISLAMBACKEND_HPP_ */
